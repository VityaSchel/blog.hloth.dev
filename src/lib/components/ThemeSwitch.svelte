...
>
	...
	<button class="group h-6 w-full cursor-pointer rounded-full" type="submit">
		<div
			class="theme-switch-bg relative h-[2px] w-full overflow-clip
				transition-all duration-200 group-hover:h-6 group-hover:rounded-xl
				group-focus-visible:h-6 group-focus-visible:rounded-xl"
			style="background-position-x: {(new Date().getHours() / 24) * 33.333 +
				'%'}"
		>
			<div
				class="theme-switch-bg-open absolute top-0 left-0 h-full w-full
			...
			>
				<span
					class="absolute top-1/2 left-1/2 block -translate-x-1/2
						-translate-y-1/2 font-display text-sm font-medium text-cream
						dark:text-black"
				>
