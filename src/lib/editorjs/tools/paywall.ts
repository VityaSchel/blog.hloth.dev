...
export default class DonationPaywallTool {
	...
	render() {
		...
		addButton.className =
			'w-fit px-2 py-1 mt-2 rounded-full bg-blue-500 text-white text-sm';
		addButton.innerHTML =
			'<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24"><path fill="currentColor" d="M11 13H6q-.425 0-.712-.288T5 12t.288-.712T6 11h5V6q0-.425.288-.712T12 5t.713.288T13 6v5h5q.425 0 .713.288T19 12t-.288.713T18 13h-5v5q0 .425-.288.713T12 19t-.712-.288T11 18z"/></svg>';
		addButton.addEventListener('click', () => {
			this.data.links = this.data.links.concat([{ url: '', title: '' }]);
			createInput({ url: '', title: '' }, this.data.links.length - 1);
		});
		addButtonContainer.appendChild(addButton);
		const paywallSpan = document.createElement('span');
		paywallSpan.className =
			'text-sm text-[#a3a3a3] dark:text-[#4b5563] flex items-center gap-1 pointer-events-none select-none';
		...
		const createInput = (value: Link, i: number) => {
			...
			titleInput.className =
				'w-full px-3 py-2 rounded-md font-normal font-text tracking-[normal]';
			...
			titleInput.addEventListener('input', (event: Event) => {
				...
				this.data.links = this.data.links.map((link, index) => {
					if (index === i) return { ...link, title: target.value };
					else return link;
				});
			});
			...
			linkInput.addEventListener('input', (event: Event) => {
				const target = event.target as HTMLInputElement;
				this.data.links = this.data.links.map((link, index) => {
					if (index === i) return { ...link, url: target.value };
					else return link;
				});
			});
